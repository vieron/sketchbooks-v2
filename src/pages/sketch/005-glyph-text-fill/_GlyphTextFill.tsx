...
function getTextLines(value: string) {
...
function buildJustifiedTextPath(font: OpenTypeFont, value: string, size: number, tracking: number, targetWidth: number) {
  ...
  const singleTokenTracking = gapCount === 0 && firstToken && firstToken.length > 1
    ? tracking + extraWidth / (firstToken.length - 1)
    : tracking;
  let cursor = 0;
...
export default function GlyphTextFill() {
  ...
  const filledGlyphs = useMemo<FilledGlyph[]>(() => {
    ...
    const outerLines = getTextLines(String(outerTypography.text).toUpperCase() || ' ');
    const lineGap = mainSize * OUTER_LINE_HEIGHT;
    let layoutBounds: Bounds | null = null;
    const placements: GlyphPlacement[] = [];

    outerLines.forEach((line, lineIndex) => {
      const baselineY = lineIndex * lineGap;
      const glyphs = outerFont.stringToGlyphs(line || ' ');
      let cursor = 0;
      ...
      for (let index = 0; index < glyphs.length; index += 1) {
        ...
        linePlacements.push({ glyph, x: cursor, y: baselineY });
        lineBounds = mergeBounds(lineBounds, buildGlyphPath(glyph, cursor, baselineY, scale).bounds);
      ...
      if (!resolvedLineBounds) {
        if (outerLines.length > 1) {
          layoutBounds = mergeBounds(layoutBounds, {
            x1: -1,
            y1: baselineY - mainSize * 0.72,
            x2: 1,
            y2: baselineY + mainSize * 0.18,
          });
        }
        return;
      }
    ...
    const bounds = layoutBounds ?? { x1: 0, y1: -mainSize, x2: 1, y2: 0 };
    ...
    return placements.flatMap((placement, glyphIndex) => {
      const path = buildGlyphPath(
        placement.glyph,
        offsetX + placement.x * fitScale,
        offsetY + placement.y * fitScale,
        finalScale,
      );
      if (!path.bounds || !path.d) return [];

      const contours = flattenGlyphContours(
        placement.glyph,
        offsetX + placement.x * fitScale,
        offsetY + placement.y * fitScale,
        finalScale,
